...
#plotting clean data
plot(close_clean_data, xlab='Years', ylab = 'stock_Close',main="Microsoft CLEANED STOCK_CLOSE DATA", type="l", lwd=2, col="red",xlim=c(1995,2019),axes=F)
axis(1,at=1995:2020,labels=1995:2020);axis(2);box()

#################### PREREQUISITE OF ARIMA MODELLING ########################################
#AUGMENTED DICKER FULLER TEST
suppressWarnings(adf.test(close_clean_data, alternative="stationary"))
ns <- ndiffs(close_clean_data)
differenced_data <- diff(close_clean_data,differences=ns)
suppressWarnings(adf.test(differenced_data, alternative="stationary"))

#plotting time series data
plot(differenced_data, xlab='Years', ylab = 'Stock_Close',main="Stationary time series" , type="l", lwd=2, col="red",xlim=c(1995,2019),axes=F)
axis(1,at=1995:2020,labels=1995:2020);axis(2);box()
...
components.ts = decompose(close_clean_data)
plot(components.ts, xlab='Years', ylab = 'Stock_Close', col="black",xlim=c(1995,2019))

close_clean_data %>% decompose %>% autoplot
